...
Polyhedron *remove_equalities_p(Polyhedron *P, unsigned nvar, Matrix **factor,
				unsigned MaxRays);
void manual_count(Polyhedron *P, Value_poly* result);
...
void Line_Length(Polyhedron *P, Value_poly *len);
struct barvinok_options;
evalue* ParamLine_Length(Polyhedron *P, Polyhedron *C,
			 struct barvinok_options *options);
void Extended_Euclid(Value_poly a, Value_poly b, Value_poly *x, Value_poly *y, Value_poly *g);
...
int line_minmax(Polyhedron *I, Value_poly *min, Value_poly *max);
void count_points_e (int pos, Polyhedron *P, int exist, int nparam,
		     Value_poly *context, Value_poly *res);
int DomainContains(Polyhedron *P, Value_poly *list_args, int len, 
...
int Last_Non_Zero(Value_poly *p, unsigned len);
...
typedef void (*for_each_lower_upper_bound_fn)
		(Matrix *M, Value_poly *lower, Value_poly *upper, void *data);
